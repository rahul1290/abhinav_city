...
if (isset($property_detail)) {
    ...
    $(document).on('click', '#corner,#garder,#calculateinper', function() {
    	totalCost();
    	checkpremium();
    });
    
    function checkpremium(){
    	if ($('#corner').prop('checked') == true || $('#garder').prop('checked') == true){
    		var counter = 0;
    		if($('#corner').prop('checked') == true){
    			counter++;
    		}
    		...
    		$('#premium_cal').css('display','block');
			if($('#category').val() == 'plot'){
				var area = $('#area').val();
				var plot_price = $('#rate_of_plot').val();
    		...
    		$('#calculateinper').prop('checked', false);
    		$('#per_amount').val(0);
    	}
    	totalCost();
	}
	...
	$(document).on('keyup','#per_amount',function(){
		checkpremium();
	});
